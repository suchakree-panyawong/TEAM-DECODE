...
from core.heuristics import (
    ...
    thai_rot_n, looks_like_thai_rotatable_text, _THAI_POLYBIUS_DEC, score_text
)
...
def decode_bacon(value: str) -> str | None:
    ...
    if len(compact) < 5 or len(compact) % 5 != 0 or not re.fullmatch(r"[AB]+", compact): return None
...
@register_decoder(name="rot13", category="cipher")
def decode_rot13(v: str) -> str | None:
    if not looks_like_rotatable_text(v): return None
    c = rot_n(v, 13); return c if c != v else None
...
@register_decoder(name="xor_multibyte", category="bitwise")
def decode_xor_multibyte(value: str) -> str | None:
    """
    1-4 Bytes Heuristic XOR Decoder
    ทำการจำลองคีย์ 1 Byte ทั้งหมด (0-255) และคีย์คำสั้นๆ ที่มักพบบ่อยใน CTF
    แล้วใช้ระบบตรวจหาอักขระที่อ่านได้ (Printable Score) เพื่อหาคำตอบที่น่าจะเป็นไปได้ที่สุด
    """
    try:
    ...
    # ทดสอบคีย์ 1 Byte ทั้งหมด 256 แบบ และคีย์ยอดนิยม (Multi-byte 2-4 ตัว)
    # เพื่อป้องกัน Loop หน่วงระบบจาก 4-bytes exhaustive 
    common_keys = [b"key", b"flag", b"ctf", b"1234", b"admin", b"root", b"0x00"]
    keys_to_try = [[i] for i in range(1, 256)] + common_keys
    
    for k in keys_to_try:
        if not k: continue
        dec = bytearray(len(raw))
        for i in range(len(raw)):
            dec[i] = raw[i] ^ k[i % len(k)]
    ...
    return best_text if best_text and best_text != value else None