...
from core.heuristics import _log_debug, bytes_to_text, is_effectively_binary, clean_separators
...
def decode_base16(value: str) -> str | None:
    compact = clean_separators(value)  # strip_dash_underscore_dot=True (default) is safe: hex alphabet never uses -_.
...
def decode_base64(value: str) -> str | None:
    compact = clean_separators(value)  # standard base64 alphabet never uses -_. as payload
...
def decode_base64url(value: str) -> str | None:
    compact = clean_separators(value, strip_dash_underscore_dot=False)
    if len(compact) < 4 or not re.fullmatch(r"[A-Za-z0-9_-]+=*", compact): return None
    try:
        raw = base64.urlsafe_b64decode(add_base64_padding(compact))
